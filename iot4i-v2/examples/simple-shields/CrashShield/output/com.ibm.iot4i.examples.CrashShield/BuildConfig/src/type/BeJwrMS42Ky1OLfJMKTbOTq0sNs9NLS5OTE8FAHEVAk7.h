...
#ifndef BEJWRMS42KY1OLFJMKTBOTQ0SNS9NLS5OTE8FAHEVAK7_H_
#define BEJWRMS42KY1OLFJMKTBOTQ0SNS9NLS5OTE8FAHEVAK7_H_
...
public:
    ...
    typedef SPL::rstring userId_type;
    typedef SPL::rstring key_type;
    typedef SPL::rstring message_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple(), userId_(), key_(), message_() {}
    SELF(const Self & ot) : Tuple(), userId_(ot.userId_), key_(ot.key_), message_(ot.message_) 
      { constructPayload(ot); }
    SELF(const userId_type & _userId, const key_type & _key, const message_type & _message) : Tuple(), userId_(_userId), key_(_key), message_(_message) { }
    ...
    userId_type & get_userId() { return userId_; }
    const userId_type & get_userId() const { return userId_; }
    void set_userId(const userId_type & _userId) { userId_ = _userId; }
    
    key_type & get_key() { return key_; }
    const key_type & get_key() const { return key_; }
    void set_key(const key_type & _key) { key_ = _key; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<userId_type >()(userId_);
        s = 37 * s + std::tr1::hash<key_type >()(key_);
    ...
    {
        size_t size = 0;
           size += userId_.getSerializedSize();
   size += key_.getSerializedSize();
   size += message_.getSerializedSize();
    ...
    void swap(SELF & ot) 
    { 
        std::swap(userId_, ot.userId_);
        std::swap(key_, ot.key_);
...
#undef SELF
#endif // BEJWRMS42KY1OLFJMKTBOTQ0SNS9NLS5OTE8FAHEVAK7_H_ 
