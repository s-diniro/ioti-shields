...
#ifndef BEJWRMSW2SUKSSQQADLECDN_H_
#define BEJWRMSW2SUKSSQQADLECDN_H_
...
#define SELF BeJwrMSw2SUksSQQADLECDn
...
public:
    ...
    typedef SPL::rstring data_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple(), data_() {}
    SELF(const Self & ot) : Tuple(), data_(ot.data_) 
      { constructPayload(ot); }
    SELF(const data_type & _data) : Tuple(), data_(_data) { }
    ...
    void set_data(const data_type & _data) { data_ = _data; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<data_type >()(data_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMSW2SUKSSQQADLECDN_H_ 
