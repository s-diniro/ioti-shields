...
#ifndef BEJWRMS42KY1OLFJMYSM2LEOSDY1LZSSPLJY0SKWUYCZPC0GSSSWTBGAMAA5M_H_
#define BEJWRMS42KY1OLFJMYSM2LEOSDY1LZSSPLJY0SKWUYCZPC0GSSSWTBGAMAA5M_H_
...
public:
    ...
    typedef SPL::list<SPL::rstring > rawEvents_type;
    typedef SPL::rstring actionParams_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple(), userId_(), rawEvents_(), actionParams_() {}
    SELF(const Self & ot) : Tuple(), userId_(ot.userId_), rawEvents_(ot.rawEvents_), actionParams_(ot.actionParams_) 
      { constructPayload(ot); }
    SELF(const userId_type & _userId, const rawEvents_type & _rawEvents, const actionParams_type & _actionParams) : Tuple(), userId_(_userId), rawEvents_(_rawEvents), actionParams_(_actionParams) { }
    ...
    rawEvents_type & get_rawEvents() { return rawEvents_; }
    const rawEvents_type & get_rawEvents() const { return rawEvents_; }
    void set_rawEvents(const rawEvents_type & _rawEvents) { rawEvents_ = _rawEvents; }
    
    actionParams_type & get_actionParams() { return actionParams_; }
    const actionParams_type & get_actionParams() const { return actionParams_; }
    void set_actionParams(const actionParams_type & _actionParams) { actionParams_ = _actionParams; }
    ...
    {
        buf << userId_ << rawEvents_ << actionParams_;
    }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<rawEvents_type >()(rawEvents_);
        s = 37 * s + std::tr1::hash<actionParams_type >()(actionParams_);
        return s;
    }
   ...
   size += rawEvents_.getSerializedSize();
   size += actionParams_.getSerializedSize();
    ...
    { 
        userId_ = ot.userId_;
        rawEvents_ = ot.rawEvents_;
        actionParams_ = ot.actionParams_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    { 
        ...
        std::swap(rawEvents_, ot.rawEvents_);
        std::swap(actionParams_, ot.actionParams_);
...
private:
    ...
    userId_type userId_;
    rawEvents_type rawEvents_;
    actionParams_type actionParams_;
...
#undef SELF
#endif // BEJWRMS42KY1OLFJMYSM2LEOSDY1LZSSPLJY0SKWUYCZPC0GSSSWTBGAMAA5M_H_ 
