	...
	public static boolean entryCondition(String message) {
		...
		try {
			...
			return (rawEvent.get("traitStates") != null
					...
					&& rawEvent.getAsJsonObject("traitStates").getAsJsonObject("traitStates").get("Temperature") != null
					&& rawEvent.getAsJsonObject("traitStates").getAsJsonObject("traitStates")
							.getAsJsonObject("Temperature").get("temperatureC") != null);
		} catch (Exception e) {
			logger.log(Level.DEBUG, "entry condition check failed, error: " + e.getLocalizedMessage());
			return false;
		}
	}
